...
**Next steps:** `NEXT_TASKS.md` lists what remains (slice measurements, full-build gates, decisions) with the commands and where to record each result.
...
| Phase 0.3: `candidate_groups / row_groups` on the largest available lake | Step 1 | Demo lake (`demo/lake/2026_01/lake/entries/`, one file, one row group): `row_groups = 1`, `candidate_groups = 1` (corrected in Step 20: the first run printed 0 because the query used `stats_min`, the legacy field PyArrow omits for strings; with `stats_min_value` the one row group is a candidate, ratio 1.0 as expected). Meaningless as a ratio; re-run on the slice is **deferred** (Step 3 not runnable in the sandbox: `rest.uniprot.org` blocked, no full dump). |
...
| Phase 3: point-lookup benchmark table; B1 kept on child tables? | Step 20 | **Deferred to the slice / full build.** `benchmarks/bench_point_lookup.py` written and smoke-run on a multi-file stress-fixture build (`--target-file-bytes 1000000 --batch-size 2000`, 9 `entries` row groups, 17 `accession_map`, 209 `features`, 536 `xrefs`) locally and over a local Range-capable HTTP server. Mechanics on the fixture: two-step `entries` lookup touches 2 `accession_map` + 2 `entries` candidate row groups (4; the ≤ 3 criterion is for 100,000-row groups), 11 range requests over HTTP (< 20: PASS); single-step touches every row group (9/9) as expected without bloom filters; two-step `features` 2/209 and `xrefs` 2/536 vs 54/209 and 63/536 single-step. The 37-accession 10× criterion cannot be judged on one-row-group-per-organism data. **B1 on child tables: moot** — no bloom filters can be written (Phase 0.1); revisit when PyArrow supports them. Findings: DuckDB's `parquet_metadata` `stats_min/stats_max` are the legacy fields (NULL for PyArrow-written strings) — every statistics query now uses `stats_min_value/stats_max_value`; wall times over a local `http.server` carry a ~1 s artefact, so remote timings need a real server (`--url`). |
| C.3: HTTP request count for the default-column organism query before/after the reorder | Step 20 | **After only** (no pre-Step-7 build exists): on the stress-fixture build over the local HTTP server, the seven-default-column `taxid = 9606` query issues 12 range requests per run. Record the slice number from `bench_point_lookup.py --serve` (C.3 line of its report). |
...
SELECT count(*) AS row_groups,
       count(*) FILTER (WHERE stats_min_value <= 'P04637' AND stats_max_value >= 'P04637') AS candidate_groups
FROM parquet_metadata('<lake>/entries/*/*.parquet') WHERE path_in_schema = 'acc';
-- stats_min_value / stats_max_value, not stats_min / stats_max: DuckDB's stats_min is the legacy
-- Parquet field, which PyArrow omits for string columns (found in Step 20, 2026-09-17).
```
...
- Write `RELEASE.metalink` in `lake/` in Metalink 4 XML (RFC 5854) (**root only** — decided 2026-09-17 while implementing Step 15: a per-table copy breaks directory reads in Polars, PyArrow and pandas, which treat every file under `<table>/` as Parquet, and the D.1 reader table takes precedence; the root file lists every Parquet file with its `<table>/review_status=…/…` path, so `grep '^entries/'`-style subsets still work): one `<file name=…>` per Parquet file with `<size>`, `<hash type="sha-256">` and `<hash type="md5">`, and `<url>` relative to the directory. This is the convention on every other UniProt FTP folder (`help/metalink.md` in `ebi-uniprot/uniprot-manual`: "Every folder on our FTP server contains a file called RELEASE.metalink"), so `curl --metalink` and existing UniProt download scripts work on the lake unchanged. MD5 is included only for metalink compatibility with those scripts; SHA-256 is the hash the validator checks. Generating it costs one more pass over the same `file_details` dict; put it next to the `SHA256SUMS.txt` writer.
...
2. **Macros stay lazy — with one correction found in Step 16 (2026-09-17).** DuckDB resolves a macro's *column references* when the macro is created, so a one-column stub (`SELECT error(...) AS _`) makes `CREATE MACRO organism_features` fail with "does not have a column named taxid". The stub therefore keeps the table's real column names from `manifest.json` (`SELECT error(...)::VARCHAR AS acc, … WHERE error(...)`); `error()` must appear both as every column and as the filter, because the optimiser folds a NULL column away (a filtered query would silently return no rows) and a bare `count(*)` touches no column. `tests/test_client.py` proves `SELECT count(*) FROM features` and `organism_features(...)` both raise `not in this lake copy` on an entries-only copy.
