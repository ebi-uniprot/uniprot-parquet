...
| D.5: per-division counts vs FTP; human-files-to-human-rows byte ratio | Step 6 / Step 3 | ______ |
...
- `tests/test_parquet_transform.py`: build from a fixture that lacks an optional path (`small.json.gz` lacks `organismHosts`) and assert the column's Arrow type is the declared one, not `int32`.
