...
**Status:** written 2026-09-16 against commit `f1c9fb6` plus the uncommitted `PLAN_SCHEMA_V2.md`. Implemented 2026-09-16/17 on `static-lake`, one commit per step (`git log --oneline --grep='^Step '`). Step 12 skipped (PyArrow 23 cannot write bloom filters). Deferred to the slice / cluster: Step 3's slice build and F.1 table, the size gates' slice runs (Step 6), the byte delta on the slice (Step 11), the F.4 / H.1 measurements (Step 18, provisional constants set), the Phase 3 run (Step 20) and Step 21's full-build gates. Corrections made while implementing are marked in place and in the plan's Results log. What remains is in `NEXT_TASKS.md`. Note: `demo/lake/` is gitignored, so Step 21.1's "commit `demo/lake/2026_01/`" is a local regeneration only.
