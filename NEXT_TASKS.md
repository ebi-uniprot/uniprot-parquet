...
| 3.4 | **Publish step (A5)**: atomic swap, `latest`/`previous`, `releases.json` (format fixed in plan F.2.5). `SHA256SUMS.txt`, `RELEASE.metalink`, `RELEASE_COMPLETE` already exist. Note: `RELEASE_COMPLETE` is not published by the `PROVENANCE` process but copied into the release directory by `workflow.onComplete` in `upjson2lake.nf` (publishDir copies are asynchronous; onComplete runs after they all finish). The A5 step should take over that copy so the marker is written after the swap, not before it. | `AUDIT.md` A5 | not started |
...
| 3.7 | **Nextflow version**: ≥ 26.04 needs `NXF_SYNTAX_PARSER=v1`; either pin < 26 in `environment.yml` or port `upjson2lake.nf` to the strict syntax: move the top-level `def release_dir` and `def complete_marker` into the workflow. `complete_marker` is set from a `subscribe` closure and read in `workflow.onComplete`, so in the strict syntax it needs another route to the handler (e.g. derive the marker path from `release_dir`/the `PROVENANCE` task dir, or a workflow-level output). | README "Setup" | env var documented |
