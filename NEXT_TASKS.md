...
| 3.4 | **Publish step (A5)**: atomic swap, `latest`/`previous`, `releases.json` (format fixed in plan F.2.5). `SHA256SUMS.txt`, `RELEASE.metalink`, `RELEASE_COMPLETE` already exist. Note: `RELEASE_COMPLETE` is published by `PROVENANCE` as `.RELEASE_COMPLETE.pending` and renamed into place by the workflow's `onComplete:` section in `upjson2lake.nf` (publishDir copies are asynchronous; onComplete runs after they all finish). The A5 step should take over that copy so the marker is written after the swap, not before it. | `AUDIT.md` A5 | not started |
...
| 3.7 | **Nextflow version**: ~~≥ 26.04 needs `NXF_SYNTAX_PARSER=v1`; pin or port~~ **Done 2026-09-17:** `upjson2lake.nf` ported to the strict syntax (script-level functions instead of top-level `def`, `onComplete:` section in the entry workflow, `RELEASE_COMPLETE` published as `.RELEASE_COMPLETE.pending` and renamed by onComplete) and pinned to Nextflow ≥ 26.04 in `environment.yml` and `nextflow.config`. Verified end to end on the demo input under the default parser. | README "Setup" | done |
...
- **Demo lake is gitignored**; `cd demo && ./run_demo.sh --clean` regenerates it (no network needed while `demo/input.json.gz` exists).
